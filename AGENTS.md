...
- **网页消息点击发送后必须先用稳定 clientId 固化为用户卡片并写入服务端持久 Outbox，桌面任务或原生队列请求超时不能直接当作发送失败。** 页面可能随时关闭，Codex 桌面 owner 也可能已启动 turn 或写入队列却没有及时返回响应；由服务端按接收顺序后台投递、幂等重试并在最终失败后通知 ClawBot，同时保留可复制和可重新提交的原消息，才能避免消息消失、乱序、重复任务和附件过早删除。
