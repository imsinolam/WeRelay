...
请先按 [从 GitHub 安装与更新 WeRelay](GitHub源码安装与更新.md) 生成本地 tarball 并完成全局安装。WeRelay 没有 npm Registry 公共包，不能使用 `npm install -g werelay@latest`。

如果命令仍不存在，请检查 npm 全局 bin 目录是否已加入 `PATH`。开发工作区的临时全局命令方式见 [开发说明](../开发协作/开发与测试.md#全局命令开发验证)。
...
安装编译工具后，先尝试重建全局安装中的原生依赖：

```bash
npm rebuild -g werelay
```

如果仍失败，按 [GitHub 安装说明](GitHub源码安装与更新.md) 重新生成 tarball 并安装。
